...
public class Playing_lvl4 extends BaseState implements GameStateInterface {
    ...
    public Playing_lvl4(Game game) {
        super(game);

        aSetter.setObject_lvl4();

        // Initialize Tilemap
        tilemap = new Tilemap(mapSpriteSheet, 4);
        ...
        player = new Player(game.getContext(), joystick, 2500, 250, 32, animator, tilemap); // 1080 x 2340  //540, 1170
    ...
    public void update() {
        ...
        if (obj[3] != null) {
            int x = 3 * TILE_WIDTH_PIXELS;
            int y = 30 * TILE_HEIGHT_PIXELS;
        ...
        if(key_count == 1 && dialog_num == 0) {
            game.setCurrentGameState(Game.GameState.DIALOG);
            dialog_num = 1;
            //return;
        }

        if(key_count == 2 && dialog_num == 1) {
            game.setDialogNum(1);
            game.setCurrentGameState(Game.GameState.DIALOG);
            dialog_num = 2;
            //return;
